...
const props = defineProps({
    ...
    // AÑADIDO: Prop para recibir los permisos desde el controlador
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <!-- HEADER MEJORADO: Con título a la izquierda y botones de acción a la derecha -->
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    Ficha de la Cooperativa
                </h2>
                <div class="flex items-center space-x-4">
                    <!-- BOTÓN DE EDITAR: Se muestra SÓLO si el usuario tiene permiso (can.update es true) -->
                    <Link
                        v-if="can.update"
                                ...
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    <tr v-for="documento in cooperativa.documentos" :key="documento.id" class="hover:bg-gray-100 dark:hover:bg-gray-900 transition-colors duration-200">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a :href="`/storage/${documento.archivo}`" target="_blank" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 mr-4">Ver</a>
